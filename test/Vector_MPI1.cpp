...
TEST_CASE("Vector<3> copy", "[Vector]")
{
  ...
  int size = (nx + 2 * num_ghost_cells) * (ny + 2 * num_ghost_cells) * (nz + 2 * num_ghost_cells) *
             num_components * num_local_patches;
...
TEST_CASE("Vector<3> add", "[Vector]")
{
  ...
  double* b_copy_data =
    &b_copy.getPatchView(0)(-num_ghost_cells, -num_ghost_cells, -num_ghost_cells, 0);
  for (size_t i = 0; i < size; i++) {
    double x = (i + 0.5) / size;
  ...
  double* expected_data =
    &expected.getPatchView(0)(-num_ghost_cells, -num_ghost_cells, -num_ghost_cells, 0);
  for (size_t i = 0; i < size; i++) {
    double x = (i + 0.5) / size;
