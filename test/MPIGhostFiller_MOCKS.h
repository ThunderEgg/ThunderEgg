...
class CallMockMPIGhostFiller : public MPIGhostFiller<D>
{
	...
	mutable std::list<std::tuple<std::shared_ptr<const PatchInfo<D>>, Side<D>, const NbrType, const Orthant<D - 1>, int, int>> nbr_calls;

	mutable std::list<std::tuple<std::shared_ptr<const PatchInfo<D>>, Edge<D>, const NbrType, const Orthant<1>, int, int>> edge_nbr_calls;

	mutable std::list<std::tuple<std::shared_ptr<const PatchInfo<D>>, Corner<D>, const NbrType, int, int>> corner_nbr_calls;
	...
	void fillGhostCellsForEdgeNbrPatch(std::shared_ptr<const PatchInfo<D>> pinfo,
	                                   const std::vector<LocalData<D>> &   local_datas,
	...
	void fillGhostCellsForCornerNbrPatch(std::shared_ptr<const PatchInfo<D>> pinfo,
	                                     const std::vector<LocalData<D>> &   local_datas,
	...
	void checkNbrCalls()
	{
		...
		= [&](const std::tuple<std::shared_ptr<const PatchInfo<D>>, Side<D>, const NbrType,
				...
				if (patch->hasNbr(side)) {
					switch (patch->getNbrType(side)) {
						case NbrType::Normal: {
							...
							auto call = make_tuple(patch, side, NbrType::Normal, Orthant<D - 1>::null(), num_components, num_components);
						...
						case NbrType::Fine: {
							...
							for (auto orthant : Orthant<D - 1>::getValues()) {
								...
								auto call = make_tuple(patch, side, NbrType::Fine, orthant, num_components, num_components);
								check_for_nbr_call(call);
							}
						} break;
						case NbrType::Coarse: {
							...
							auto call = make_tuple(patch, side, NbrType::Coarse, orthant, num_components, num_components);
							check_for_nbr_call(call);
	...
	void checkEdgeNbrCalls()
	{
		...
		auto check_for_nbr_call
		= [&](const std::tuple<std::shared_ptr<const PatchInfo<D>>, Edge<D>, const NbrType,
				...
				if (patch->hasEdgeNbr(edge)) {
					switch (patch->getEdgeNbrType(edge)) {
						case NbrType::Normal: {
							...
							auto call = make_tuple(patch, edge, NbrType::Normal, Orthant<1>::null(), num_components, num_components);
							check_for_nbr_call(call);
						...
						case NbrType::Fine: {
							...
							for (auto orthant : Orthant<D - 1>::getValues()) {
								INFO("Orthant: " << orthant.getIndex());
								auto call = make_tuple(patch, edge, NbrType::Fine, orthant, num_components, num_components);
								check_for_nbr_call(call);
							}
						} break;
						case NbrType::Coarse: {
							...
							auto call = make_tuple(patch, edge, NbrType::Coarse, orthant, num_components, num_components);
							check_for_nbr_call(call);
	...
	void checkCornerNbrCalls()
	{
		...
		auto check_for_nbr_call
		= [&](const std::tuple<std::shared_ptr<const PatchInfo<D>>, Corner<D>, const NbrType,
		                       int, int> &call) {
			  // check if call was made for neighbor
			  auto found_call
			  = std::find(remaining_nbr_calls.begin(), remaining_nbr_calls.end(), call);
				...
				if (patch->hasCornerNbr(corner)) {
					switch (patch->getCornerNbrType(corner)) {
						case NbrType::Normal: {
							...
							auto call = make_tuple(patch, corner, NbrType::Normal, num_components, num_components);
							check_for_nbr_call(call);
						...
						case NbrType::Fine: {
							...
							auto call = make_tuple(patch, corner, NbrType::Fine, num_components, num_components);
							check_for_nbr_call(call);
						} break;
						case NbrType::Coarse: {
							INFO("NbrType: Coarse");

							auto call = make_tuple(patch, corner, NbrType::Coarse, num_components, num_components);
							check_for_nbr_call(call);
