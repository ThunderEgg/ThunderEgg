...
TEST_CASE("Test Poisson::DFTPatchSolver gets 2nd order convergence", "[Poisson::DFTPatchSolver]")
{
...
TEST_CASE("Test Poisson::DFTPatchSolver gets 2nd order convergence with neumann boundary", "[Poisson::DFTPatchSolver]")
{
  for (auto mesh_file : { MESHES }) {
    for (auto nx : { 10, 13 }) {
      for (auto ny : { 10, 13 }) {
...
TEST_CASE("Test Poisson::DFTPatchSolver gets 2nd order convergence with neumann boundary single patch", "[Poisson::DFTPatchSolver]")
{
  for (auto nx : { 10, 13 }) {
    for (auto ny : { 10, 13 }) {
