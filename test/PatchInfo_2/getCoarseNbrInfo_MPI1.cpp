...
TEST_CASE("PatchInfo<2> getCoarseNbrInfo side")
{
  for (Side<2> s : Side<2>::getValues()) {
    ...
    CHECK_EQ(&pinfo.getCoarseNbrInfo(s), nbr_info);
  }
}

TEST_CASE("PatchInfo<2> getCoarseNbrInfo corner")
{
...
TEST_CASE("PatchInfo<2> getCoarseNbrInfo side throws on null")
{
  for (Side<2> s : Side<2>::getValues()) {
    PatchInfo<2> pinfo;
...
TEST_CASE("PatchInfo<2> getCoarseNbrInfo corner throws on null")
{
  for (Corner<2> c : Corner<2>::getValues()) {
    PatchInfo<2> pinfo;
...
TEST_CASE("PatchInfo<2> getCoarseNbrInfo side throws on wrong type")
{
  for (Side<2> s : Side<2>::getValues()) {
    PatchInfo<2> pinfo;
...
TEST_CASE("PatchInfo<2> getCoarseNbrInfo corner throws on wrong type")
{
  for (Corner<2> c : Corner<2>::getValues()) {
    PatchInfo<2> pinfo;
