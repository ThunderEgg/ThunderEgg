  ...
  for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
    ...
    for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
      ...
      for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
        ...
        CHECK(&ca[{ nx - 1 - xi, yi, zi }] ==
              &(ca.getSliceOn(Corner<3>::bse(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ xi, ny - 1 - yi, zi }] ==
              &(ca.getSliceOn(Corner<3>::bnw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx - 1 - xi, ny - 1 - yi, zi }] ==
              &(ca.getSliceOn(Corner<3>::bne(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ xi, yi, nz - 1 - zi }] ==
              &(ca.getSliceOn(Corner<3>::tsw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx - 1 - xi, yi, nz - 1 - zi }] ==
              &(ca.getSliceOn(Corner<3>::tse(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ xi, ny - 1 - yi, nz - 1 - zi }] ==
              &(ca.getSliceOn(Corner<3>::tnw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx - 1 - xi, ny - 1 - yi, nz - 1 - zi }] ==
              &(ca.getSliceOn(Corner<3>::tne(), { xi, yi, zi })[{}]));
      }
    }
  }
}
...
TEST_CASE("ComponentArray<3> getGhostSliceOn<0>", "[ComponentArray]")
{
  ...
  for (unsigned char zi = 0; zi < num_ghost; zi++) {
    ...
    for (unsigned char yi = 0; yi < num_ghost; yi++) {
      ...
      for (unsigned char xi = 0; xi < num_ghost; xi++) {
        INFO("xi: " << xi);
        CHECK(&ca[{ -1 - xi, -1 - yi, -1 - zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::bsw(), { xi, yi, zi }))[{}]);
        CHECK(&ca[{ nx + xi, -1 - yi, -1 - zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::bse(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ -1 - xi, ny + yi, -1 - zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::bnw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx + xi, ny + yi, -1 - zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::bne(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ -1 - xi, -1 - yi, nz + zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::tsw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx + xi, -1 - yi, nz + zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::tse(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ -1 - xi, ny + yi, nz + zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::tnw(), { xi, yi, zi })[{}]));
        CHECK(&ca[{ nx + xi, ny + yi, nz + zi }] ==
              &(ca.getGhostSliceOn(Corner<3>::tne(), { xi, yi, zi })[{}]));
      }
    }
  }
}
