...
PatchVector::PatchVector(const ThunderEgg::Domain<3>& domain, int max_level)
  : max_level(max_level)
  , pinfos(GetAllPatchesOnRank0(domain))
{
  int rank;
...
const ThunderEgg::PatchInfo<3>*
PatchVector::operator[](const std::string& str) const
{
  int i = 0;
  int j = 0;
  int k = 0;
...
GetAllPatchesOnRank0(const Domain<3>& domain)
{
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
namespace {
std::string
getString(int max_level, int i, int j, int k)
{
  std::string str;
...
Orthant<3>
getChildOrthant(const string& str)
{
  int val = 0;
...
void
Check4x4x4DomainSideHasNeighbors(const PatchVector& domain);
void
