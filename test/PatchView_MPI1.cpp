...
TEST_CASE("PatchView<double,3> getSliceOn<2>", "[PatchView]")
{
  ...
  array<int, 4> strides = { 1,
                            nx + 2 * num_ghost,
                            (nx + 2 * num_ghost) * (ny + 2 * num_ghost),
                            (nx + 2 * num_ghost) * (ny + 2 * num_ghost) * (nz + 2 * num_ghost) };
  int size = 1;
  ...
  for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
    ...
    CHECK(slice.getGhostEnd() ==
          array<int, 3>({ ny - 1 + num_ghost, nz - 1 + num_ghost, num_components - 1 }));
  ...
  for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
    ...
    CHECK(slice.getGhostEnd() ==
          array<int, 3>({ nx - 1 + num_ghost, nz - 1 + num_ghost, num_components - 1 }));
  ...
  for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
    ...
    CHECK(slice.getGhostEnd() ==
          array<int, 3>({ nx - 1 + num_ghost, ny - 1 + num_ghost, num_components - 1 }));
    ...
    for (int yi = -num_ghost - 1; yi < ny + num_ghost + 1; yi++) {
      for (int xi = -num_ghost - 1; xi < nx + num_ghost + 1; xi++) {
        if (xi < -num_ghost || xi >= nx + num_ghost || yi < -num_ghost || yi >= ny + num_ghost ||
            c < 0 || c >= num_components) {
          // oob coord
          if constexpr (ENABLE_DEBUG) {
            CHECK_THROWS_AS((v[{ xi, yi, c }]), RuntimeError);
          }
        } else {
          CHECK(&v[{ xi, yi, c }] == start + xi + yi * (nx + 2 * num_ghost) +
                                       c * (ny + 2 * num_ghost) * (nx + 2 * num_ghost));
        }
      }
    }
  }
    ...
    for (int yi = -num_ghost - 1; yi < ny + num_ghost + 1; yi++) {
      for (int xi = -num_ghost - 1; xi < nx + num_ghost + 1; xi++) {
        if ((xi < -num_ghost) || (xi >= nx + num_ghost) || (yi < -num_ghost) ||
            (yi >= ny + num_ghost) || c < 0 || c >= num_components) {
          // oob coord
          if constexpr (ENABLE_DEBUG) {
...
TEST_CASE("PatchView<double,3> getComponentView", "[PatchView]")
{
  ...
  for (int c = 0; c < num_components; c++) {
    ...
    CHECK(slice.getGhostEnd() ==
          array<int, 3>({ nx - 1 + num_ghost, ny - 1 + num_ghost, nz - 1 + num_ghost }));
