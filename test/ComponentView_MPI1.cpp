  ...
  for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
    ...
    for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
      ...
      for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
        ...
        CHECK(&ld[{ nx - 1 - xi, yi, zi }] ==
              &(ld.getSliceOn(Corner<3>::bse(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ xi, ny - 1 - yi, zi }] ==
              &(ld.getSliceOn(Corner<3>::bnw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx - 1 - xi, ny - 1 - yi, zi }] ==
              &(ld.getSliceOn(Corner<3>::bne(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ xi, yi, nz - 1 - zi }] ==
              &(ld.getSliceOn(Corner<3>::tsw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx - 1 - xi, yi, nz - 1 - zi }] ==
              &(ld.getSliceOn(Corner<3>::tse(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ xi, ny - 1 - yi, nz - 1 - zi }] ==
              &(ld.getSliceOn(Corner<3>::tnw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx - 1 - xi, ny - 1 - yi, nz - 1 - zi }] ==
              &(ld.getSliceOn(Corner<3>::tne(), { xi, yi, zi })[{}]));
      }
    }
  }
}
...
TEST_CASE("ComponentView<double,3> getGhostSliceOn<0>", "[ComponentView]")
{
  ...
  for (unsigned char zi = 0; zi < num_ghost; zi++) {
    ...
    for (unsigned char yi = 0; yi < num_ghost; yi++) {
      ...
      for (unsigned char xi = 0; xi < num_ghost; xi++) {
        INFO("xi: " << xi);
        CHECK(&ld[{ -1 - xi, -1 - yi, -1 - zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::bsw(), { xi, yi, zi }))[{}]);
        CHECK(&ld[{ nx + xi, -1 - yi, -1 - zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::bse(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ -1 - xi, ny + yi, -1 - zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::bnw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx + xi, ny + yi, -1 - zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::bne(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ -1 - xi, -1 - yi, nz + zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::tsw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx + xi, -1 - yi, nz + zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::tse(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ -1 - xi, ny + yi, nz + zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::tnw(), { xi, yi, zi })[{}]));
        CHECK(&ld[{ nx + xi, ny + yi, nz + zi }] ==
              &(ld.getGhostSliceOn(Corner<3>::tne(), { xi, yi, zi })[{}]));
      }
    }
  }
}
...
TEST_CASE("ComponentView set", "[ComponentView]")
{
  ...
  for (int yi = -num_ghost - 1; yi < ny + num_ghost + 1; yi++) {
    for (int xi = -num_ghost - 1; xi < nx + num_ghost + 1; xi++) {
      if ((xi < -num_ghost) || (xi >= nx + num_ghost) || (yi < -num_ghost) ||
          (yi >= ny + num_ghost)) {
        // oob coord
        if constexpr (ENABLE_DEBUG) {
...
TEST_CASE("ComponentView set const", "[ComponentView]")
{
  ...
  for (int yi = -num_ghost - 1; yi < ny + num_ghost + 1; yi++) {
    for (int xi = -num_ghost - 1; xi < nx + num_ghost + 1; xi++) {
      ...
      } else if (xi < -num_ghost || xi >= nx + num_ghost || yi < -num_ghost ||
                 yi >= ny + num_ghost) {
        // oob coord
        if constexpr (ENABLE_DEBUG) {
          CHECK_THROWS_AS(v.set({ xi, yi }, value), RuntimeError);
