...
TEST_CASE("PatchArray<2> getSliceOn<1> const", "[PatchArray]")
{
	...
	for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
		INFO("xi: " << xi);
		const View<1> slice = pa.getSliceOn(Side<2>::west(), {xi});
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			CHECK(&pa[{xi, yi}] == &slice[{yi}]);
		}
	...
	for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
		INFO("xi: " << xi);
		const View<1> slice = pa.getSliceOn(Side<2>::east(), {xi});
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			CHECK(&pa[{nx - 1 - xi, yi}] == &slice[{yi}]);
		}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		INFO("yi: " << yi);
		const View<1> slice = pa.getSliceOn(Side<2>::south(), {yi});
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			CHECK(&pa[{xi, yi}] == &slice[{xi}]);
		}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		INFO("yi: " << yi);
		const View<1> slice = pa.getSliceOn(Side<2>::north(), {yi});
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			CHECK(&pa[{xi, ny - 1 - yi}] == &slice[{xi}]);
		}
...
TEST_CASE("PatchArray<3> getSliceOn<2> const", "[PatchArray]")
{
	...
	for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
		INFO("xi: " << xi);
		const View<2> slice = pa.getSliceOn(Side<3>::west(), {xi});
		for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
			INFO("zi: " << zi);
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
	...
	for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
		INFO("xi: " << xi);
		const View<2> slice = pa.getSliceOn(Side<3>::east(), {xi});
		for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
			INFO("zi: " << zi);
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		INFO("yi: " << yi);
		const View<2> slice = pa.getSliceOn(Side<3>::south(), {yi});
		for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
			INFO("zi: " << zi);
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		INFO("yi: " << yi);
		const View<2> slice = pa.getSliceOn(Side<3>::north(), {yi});
		for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
			INFO("zi: " << zi);
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		INFO("zi: " << zi);
		const View<2> slice = pa.getSliceOn(Side<3>::bottom(), {zi});
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		INFO("zi: " << zi);
		const View<2> slice = pa.getSliceOn(Side<3>::top(), {zi});
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
...
TEST_CASE("PatchArray<3> getSliceOn<1> const", "[PatchArray]")
{
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			const View<1> slice = pa.getSliceOn(Edge::bs(), {yi, zi});
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
				CHECK(&pa[{xi, yi, zi}] == &slice[{xi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			const View<1> slice = pa.getSliceOn(Edge::tn(), {yi, zi});
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
				CHECK(&pa[{xi, ny - 1 - yi, nz - 1 - zi}] == &slice[{xi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			const View<1> slice = pa.getSliceOn(Edge::bn(), {yi, zi});
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
				CHECK(&pa[{xi, ny - 1 - yi, zi}] == &slice[{xi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
			INFO("yi: " << yi);
			const View<1> slice = pa.getSliceOn(Edge::ts(), {yi, zi});
			for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
				INFO("xi: " << xi);
				CHECK(&pa[{xi, yi, nz - 1 - zi}] == &slice[{xi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::bw(), {xi, zi});
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
				CHECK(&pa[{xi, yi, zi}] == &slice[{yi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::te(), {xi, zi});
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
				CHECK(&pa[{nx - 1 - xi, yi, nz - 1 - zi}] == &slice[{yi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::be(), {xi, zi});
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
				CHECK(&pa[{nx - 1 - xi, yi, zi}] == &slice[{yi}]);
			}
	...
	for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::tw(), {xi, zi});
			for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
				INFO("yi: " << yi);
				CHECK(&pa[{xi, yi, nz - 1 - zi}] == &slice[{yi}]);
			}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::sw(), {xi, yi});
			for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
				INFO("zi: " << zi);
				CHECK(&pa[{xi, yi, zi}] == &slice[{zi}]);
			}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::ne(), {xi, yi});
			for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
				INFO("zi: " << zi);
				CHECK(&pa[{nx - 1 - xi, ny - 1 - yi, zi}] == &slice[{zi}]);
			}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::se(), {xi, yi});
			for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
				INFO("zi: " << zi);
				CHECK(&pa[{nx - 1 - xi, yi, zi}] == &slice[{zi}]);
			}
	...
	for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
		...
		for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
			INFO("xi: " << xi);
			const View<1> slice = pa.getSliceOn(Edge::nw(), {xi, yi});
			for (int zi = -num_ghost; zi < nz + num_ghost; zi++) {
				INFO("zi: " << zi);
				CHECK(&pa[{xi, ny - 1 - yi, zi}] == &slice[{zi}]);
			}
