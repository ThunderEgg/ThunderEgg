  ...
  for (unsigned char ci = 0; ci < num_components; ci++) {
    for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
      for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
        CHECK(&pa[{ xi, yi, ci }] == start + xi + yi * (nx + 2 * num_ghost) +
                                       ci * (nx + 2 * num_ghost) * (ny + 2 * num_ghost));
      }
    }
  }
}
    ...
    for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
      ...
      for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
        ...
        for (unsigned char ci = 0; ci < num_components; ci++) {
          ...
          CHECK(&pa(nx - 1 - xi, yi, zi, ci) ==
                &(pa.getSliceOn(Corner<3>::bse(), { xi, yi, zi })(ci)));
          CHECK(&pa(xi, ny - 1 - yi, zi, ci) ==
                &(pa.getSliceOn(Corner<3>::bnw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx - 1 - xi, ny - 1 - yi, zi, ci) ==
                &(pa.getSliceOn(Corner<3>::bne(), { xi, yi, zi })(ci)));
          CHECK(&pa(xi, yi, nz - 1 - zi, ci) ==
                &(pa.getSliceOn(Corner<3>::tsw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx - 1 - xi, yi, nz - 1 - zi, ci) ==
                &(pa.getSliceOn(Corner<3>::tse(), { xi, yi, zi })(ci)));
          CHECK(&pa(xi, ny - 1 - yi, nz - 1 - zi, ci) ==
                &(pa.getSliceOn(Corner<3>::tnw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx - 1 - xi, ny - 1 - yi, nz - 1 - zi, ci) ==
                &(pa.getSliceOn(Corner<3>::tne(), { xi, yi, zi })(ci)));
        }
      }
    }
  }
  ...
  for (int yi = -num_ghost; yi < ny + num_ghost; yi++) {
    ...
    for (int xi = -num_ghost; xi < nx + num_ghost; xi++) {
      ...
      for (unsigned char ci = 0; ci < num_components; ci++) {
        ...
        CHECK(&pa(nx - 1 - xi, ny - 1 - yi, ci) ==
              &(pa.getSliceOn(Corner<2>::ne(), { xi, yi })(ci)));
      }
    }
  }
}
  ...
  for (unsigned char zi = 0; zi < num_ghost; zi++) {
    ...
    for (unsigned char yi = 0; yi < num_ghost; yi++) {
      ...
      for (unsigned char xi = 0; xi < num_ghost; xi++) {
        ...
        for (unsigned char ci = 0; ci < num_components; ci++) {
          INFO("ci: " << ci);
          CHECK(&pa(-1 - xi, -1 - yi, -1 - zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::bsw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx + xi, -1 - yi, -1 - zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::bse(), { xi, yi, zi })(ci)));
          CHECK(&pa(-1 - xi, ny + yi, -1 - zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::bnw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx + xi, ny + yi, -1 - zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::bne(), { xi, yi, zi })(ci)));
          CHECK(&pa(-1 - xi, -1 - yi, nz + zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::tsw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx + xi, -1 - yi, nz + zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::tse(), { xi, yi, zi })(ci)));
          CHECK(&pa(-1 - xi, ny + yi, nz + zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::tnw(), { xi, yi, zi })(ci)));
          CHECK(&pa(nx + xi, ny + yi, nz + zi, ci) ==
                &(pa.getGhostSliceOn(Corner<3>::tne(), { xi, yi, zi })(ci)));
        }
      }
    }
  }
