...
TEST_CASE("Vector<3> getNumGhostCells unmanaged constructor", "[Vector]")
{
  ...
  array<int, 4> strides = { 1,
                            nx + 2 * num_ghost_cells,
                            (nx + 2 * num_ghost_cells) * (ny + 2 * num_ghost_cells),
                            (nz + 2 * num_ghost_cells) * (nx + 2 * num_ghost_cells) *
                              (ny + 2 * num_ghost_cells) };
  int patch_stride = (nz + 2 * num_ghost_cells) * (ny + 2 * num_ghost_cells) *
                     (nx + 2 * num_ghost_cells) * num_components;
  double data[patch_stride * num_local_patches];
  vector<double*> patch_starts(num_local_patches);
  for (int i = 0; i < num_local_patches; i++) {
    patch_starts[i] = data + i * patch_stride;
