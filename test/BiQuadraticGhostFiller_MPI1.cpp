...
TEST_CASE("exchange uniform 2D quad BiQuadraticGhostFiller", "[BiQuadraticGhostFiller]")
{
  ...
  // patch 1
  {
    ...
    // check that west values are not modified
    {
      ...
      Loop::Nested<1>(west_ghost.getStart(),
                      west_ghost.getEnd(),
    ...
    // check that south values are not modified
    {
      ...
      Loop::Nested<1>(south_ghost.getStart(),
                      south_ghost.getEnd(),
  ...
  // patch 2
  {
    ...
    // check that east values are not modified
    {
      ...
      Loop::Nested<1>(east_ghost.getStart(),
                      east_ghost.getEnd(),
  ...
  // patch 3
  {
    ...
    // check that north values are correct
    {
      ...
      Loop::Nested<1>(north_ghost.getStart(),
                      north_ghost.getEnd(),
...
TEST_CASE(
  "exchange various meshes 2D BiQuadraticGhostFiller ghost already set two components corners",
  "[BiQuadraticGhostFiller]")
{
  auto mesh_file =
    GENERATE(as<std::string>{}, single_mesh_file, refined_mesh_file, cross_mesh_file);
