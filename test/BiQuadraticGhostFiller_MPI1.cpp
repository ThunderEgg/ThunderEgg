...
TEST_CASE("Test BiQuadraticGhostFiller::Generator", "[Poisson::StarPatchOperator]")
{
	auto mesh_file
	= GENERATE(as<std::string>{}, single_mesh_file, refined_mesh_file, cross_mesh_file);
	...
	shared_ptr<ValVector<2>> vec      = ValVector<2>::GetNewVector(d_coarse);
	shared_ptr<ValVector<2>> expected = ValVector<2>::GetNewVector(d_coarse);

	DomainTools<2>::setValues(d_coarse, vec, f);
	DomainTools<2>::setValuesWithGhost(d_coarse, expected, f);

	auto blgf_fine = make_shared<BiQuadraticGhostFiller>(d_fine);
	auto blgf_gen  = BiQuadraticGhostFiller::Generator(blgf_fine);
	auto blgf      = blgf_gen(coarser_level);
	blgf->fillGhost(vec);
