...
MatrixHelper::MatrixHelper(const Domain<3>& domain, std::bitset<6> neumann)
  : domain(domain)
  , neumann(neumann)
{
...
public:
  ...
  const double* coeffs(int xi, int yi) { return &coeff; }
...
public:
  ...
  int row(int xi, int yi) { return start + stridex * xi + stridey * yi; }
  int size(int xi, int yi) { return (int)coeff.size(); }
  const double* coeffs(int xi, int yi) { return &coeff[0]; }
  const int* cols(int xi, int yi)
  {
    switch (quad) {
      case 0:
 ...
 */
std::unique_ptr<StencilHelper>
getStencilHelper(const PatchInfo<3>& pinfo, Side<3> s, std::bitset<6> neumann)
{
 ...
 */
static void
addCenterCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addWestCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addEastCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addNorthCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addSouthCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addTopCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
 ...
 */
static void
addBottomCoefficients(Mat A, const PatchInfo<3>& pinfo)
{
  int nx = pinfo.ns[0];
  int ny = pinfo.ns[1];
  int nz = pinfo.ns[2];
