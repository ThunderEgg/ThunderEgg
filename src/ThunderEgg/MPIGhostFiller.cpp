...
private:
  ...
  public:
     ...
     */
    bool
    operator<(const RemoteCallPrototype& other) const
    {
  ...
  public:
     ...
     */
    bool
    operator<(const IncomingGhostPrototype& other) const
    {
  ...
  public:
     ...
     */
    size_t
    getSize(Face<D, M> face) const
    {
      return sizes[face.getIndex()];
    }

    PatchView<const double, D>
    getPatchView(double* buffer_ptr, Face<D, M> face, int num_components) const
    {
  ...
  template<int M>
  PatchView<const double, D>
  getPatchViewForBuffer(double* buffer_ptr, Face<D, M> face, int num_components) const
  {
  ...
  postRecvs(std::vector<std::vector<double>>& buffers) const
  {
  ...
  template<int M>
  void
  addRecvBufferToGhost(const RemoteCallSet& remote_call_set,
   ...
   */
  void
  processRecvs(std::vector<MPI_Request>& requests,
   ...
   *
   * @tparam M the dimension of the face
   * @param filler the out class (for vtable access)
  ...
  template<int M>
  void
  fillSendBuffer(const MPIGhostFiller<D>& filler,
                 ...
                 const Vector<D>& u) const
  {
    for (const RemoteCall<M>& call : remote_call_set.remote_calls.template get<M>()) {
      ...
      fillGhostCellsForNbrPatchPriv(
        filler, pinfo, local_view, buffer_view, call.face, call.nbr_type, call.orthant);
    }
  }
   ...
   */
  std::vector<MPI_Request>
  postSends(const MPIGhostFiller<D>& filler,
            std::vector<std::vector<double>>& buffers,
            const Vector<D>& u) const
  {
    ...
    for (size_t i = 0; i < remote_call_sets.size(); i++) {
      switch (fill_type) {
        case GhostFillingType::Corners:
          if constexpr (D >= 2) {
            fillSendBuffer<0>(filler, remote_call_sets[i], buffers[i], u);
          }
        case GhostFillingType::Edges:
          if constexpr (D == 3) {
            fillSendBuffer<1>(filler, remote_call_sets[i], buffers[i], u);
          }
        case GhostFillingType::Faces:
          fillSendBuffer<D - 1>(filler, remote_call_sets[i], buffers[i], u);
          break;
        default:
          throw RuntimeError("Unsupported GhostFilling Type");
      }
  ...
  template<int M>
  void
  fillGhostCellsForNbrPatchPriv(const MPIGhostFiller<D>& filler,
  ...
  {
    if constexpr (M == D - 1) {
      filler.fillGhostCellsForNbrPatch(
        pinfo, local_view, nbr_view, face, nbr_type, orthant_on_coarse);
    } else if constexpr (M > 0) {
      filler.fillGhostCellsForEdgeNbrPatch(
        pinfo, local_view, nbr_view, face, nbr_type, orthant_on_coarse);
    } else {
      filler.fillGhostCellsForCornerNbrPatch(pinfo, local_view, nbr_view, face, nbr_type);
    }
  }
  ...
  template<int M>
  void
  processLocalFills(const MPIGhostFiller<D>& filler, const Vector<D>& u) const
  {
    for (const LocalCall<M>& call : local_calls.template get<M>()) {
      ...
      fillGhostCellsForNbrPatchPriv(
        filler, pinfo, local_view, nbr_view, call.face, call.nbr_type, call.orthant);
    }
    if constexpr (M > 0) {
      processLocalFills<M - 1>(filler, u);
    }
  }
  ...
  template<int M>
  void
  enumerateCalls(std::deque<LocalCall<M>>& my_local_calls,
  ...
  template<int M>
  void
  zeroGhostCellsOnAllFaces(const PatchInfo<D>& pinfo, PatchView<const double, D>& view) const
  {
    for (Face<D, M> f : Face<D, M>::getValues()) {
      if (pinfo.hasNbr(f)) {
        std::array<size_t, D - M> start;
   ...
   */
  void
  zeroGhostCells(const Vector<D>& u) const
  {
    for (const PatchInfo<D>& pinfo : domain.getPatchInfoVector()) {
...
public:
   ...
   */
  MPIGhostFillerImpl(const Domain<D>& domain, GhostFillingType fill_type)
    : domain(domain)
    , fill_type(fill_type)
  {
   ...
   */
  void
  fillGhost(const MPIGhostFiller<D>& filler, const Vector<D>& u) const
  {
    ...
    std::vector<MPI_Request> send_requests = postSends(filler, out_buffers, u);

    // perform local operations
    for (const PatchInfo<D>& pinfo : domain.getPatchInfoVector()) {
      PatchView<const double, D> view = u.getPatchView(pinfo.local_index);
      filler.fillGhostCellsForLocalPatch(pinfo, view);
    }
    processLocalFills<D - 1>(filler, u);
...
template<int D>
MPIGhostFiller<D>::MPIGhostFiller(const Domain<D>& domain, GhostFillingType fill_type)
  : implimentation(new MPIGhostFillerImpl<D>(domain, fill_type))
{
}

template<int D>
void
MPIGhostFiller<D>::fillGhost(const Vector<D>& u) const
{
  implimentation->fillGhost(*this, u);
}

template<int D>
GhostFillingType
MPIGhostFiller<D>::getFillType() const
{
  return implimentation->getFillType();
}

template<int D>
const Domain<D>&
MPIGhostFiller<D>::getDomain() const
{
  return implimentation->getDomain();
}

template class MPIGhostFiller<2>;
template class MPIGhostFiller<3>;
} // namespace ThunderEgg
