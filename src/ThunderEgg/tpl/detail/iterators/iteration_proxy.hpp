...
auto
get(const ThunderEgg::tpl::nlohmann::detail::iteration_proxy_value<IteratorType>& i)
  -> decltype(i.key())
{
  return i.key();
}
...
public:
  using type = decltype(get<N>(
    std::declval<::ThunderEgg::tpl::nlohmann::detail::iteration_proxy_value<IteratorType>>()));
};
#if defined(__clang__)
