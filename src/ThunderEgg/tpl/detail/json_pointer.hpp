...
public:
  ...
  std::string to_string() const
  {
    return std::accumulate(
      ...
      std::string{},
      [](const std::string& a, const std::string& b) { return a + "/" + detail::escape(b); });
  }
...
private:
  ...
  */
  static typename BasicJsonType::size_type array_index(const std::string& s)
  {
    ...
    {
      JSON_THROW(detail::out_of_range::create(
        410, "array index " + s + " exceeds size_type", BasicJsonType())); // LCOV_EXCL_LINE
    }
    ...
    for (
      ...
      for (std::size_t pos = reference_token.find_first_of('~'); pos != std::string::npos;
        ...
        if (JSON_HEDLEY_UNLIKELY(
          ...
          JSON_THROW(detail::parse_error::create(
            108, 0, "escape character '~' must be followed with '0' or '1'", BasicJsonType()));
        }
      }
