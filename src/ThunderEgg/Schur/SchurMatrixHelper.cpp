...
void SchurMatrixHelper::assembleMatrix(inserter insertBlock)
{
	...
	for (auto &p : sh->getIfaces()) {
		...
		for (size_t idx = 0; idx < ifs.sinfos.size(); i++) {
			...
			PatchIfaceInfo<3> &sinfo = *ifs.sinfos[idx];
	...
	std::shared_ptr<Vector<3>> u_vec(new PETSc::VecWrapper<3>(u, {n, n, n}, 0, false));
	std::shared_ptr<Vector<3>> f_vec(new PETSc::VecWrapper<3>(f, {n, n, n}, 0, false));
	std::shared_ptr<Vector<3>> r_vec(new PETSc::VecWrapper<3>(r, {n, n, n}, 0, false));
	std::shared_ptr<Vector<3>> e_vec(new PETSc::VecWrapper<3>(e, {n, n, n}, 0, false));
	std::shared_ptr<Vector<2>> interp_vec(new PETSc::VecWrapper<2>(interp, {n, n}, 0, false));
	std::shared_ptr<Vector<2>> gamma_vec(new PETSc::VecWrapper<2>(gamma, {n, n}, 0, false));
	...
	while (!blocks.empty()) {
		...
		std::shared_ptr<PatchIfaceInfo<3>> sd(new PatchIfaceInfo<3>(pinfo));
		pinfo->ns.fill(n);
		pinfo->spacings.fill(1.0 / n);
		pinfo->neumann = curr_type.neumann;
		// sd->getIfaceInfoPtr(Side<3>::west()).reset(new NormalIfaceInfo<3>());
		// solver->addPatch(*sd);
		std::vector<std::shared_ptr<PatchIfaceInfo<3>>> single_domain;
