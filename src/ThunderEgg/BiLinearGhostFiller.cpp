 ...
 */
void FillGhostForCoarseNbr(const std::vector<ComponentView<const double, 2>> &local_datas,
                           const std::vector<ComponentView<const double, 2>> &nbr_datas,
                           const Side<2>                                      side,
 ...
 */
void FillGhostForFineNbr(const std::vector<ComponentView<const double, 2>> &local_datas,
                         const std::vector<ComponentView<const double, 2>> &nbr_datas,
                         const Side<2>                                      side,
                         const Orthant<1>                                   orthant)
{
 ...
 * @param side the side that the neighbor patch is on
 */
void FillLocalGhostsForCoarseNbr(const PatchInfo<2> &pinfo, const ComponentView<const double, 2> &local_data, const Side<2> side)
{
	View<const double, 1> local_slice  = local_data.getSliceOn(side, {0});
 ...
 * @param side the side that the neighbor patch is on
 */
void FillLocalGhostsForFineNbr(const ComponentView<const double, 2> &local_data, const Side<2> side)
{
	View<const double, 1> local_slice  = local_data.getSliceOn(side, {0});
	View<double, 1>       local_ghosts = local_data.getGhostSliceOn(side, {0});
	nested_loop<1>(
 ...
 */
void FillGhostForCornerNormalNbr(const std::vector<ComponentView<const double, 2>> &local_datas,
                                 const std::vector<ComponentView<const double, 2>> &nbr_datas,
                                 Corner<2>                                          corner)
{
	for (size_t c = 0; c < local_datas.size(); c++) {
		View<const double, 0> local_slice = local_datas[c].getSliceOn(corner, {0, 0});
		View<double, 0>       nbr_ghost   = nbr_datas[c].getGhostSliceOn(corner.opposite(), {0, 0});
 ...
 * @param corner the corner that the neighbor patch is on
 */
void FillGhostForCornerCoarseNbr(const std::vector<ComponentView<const double, 2>> &local_datas,
                                 const std::vector<ComponentView<const double, 2>> &nbr_datas,
                                 Corner<2>                                          corner)
{
	for (size_t c = 0; c < local_datas.size(); c++) {
 ...
 * @param corner the corner that the neighbor patch is on
 */
void FillGhostForCornerFineNbr(const std::vector<ComponentView<const double, 2>> &local_datas,
                               const std::vector<ComponentView<const double, 2>> &nbr_datas,
                               Corner<2>                                          corner)
{
	for (size_t c = 0; c < local_datas.size(); c++) {
 ...
 * @param corner the corner that the neighbor patch is on
 */
void FillLocalGhostsForCornerCoarseNbr(const ComponentView<const double, 2> &local_data, Corner<2> corner)
{
	View<const double, 0> local_slice  = local_data.getSliceOn(corner, {0, 0});
	View<double, 0>       local_ghosts = local_data.getGhostSliceOn(corner, {0, 0});
 ...
 * @param corner the corner that the neighbor patch is on
 */
void FillLocalGhostsForCornerFineNbr(const ComponentView<const double, 2> &local_data, Corner<2> corner)
{
	View<const double, 0> local_slice  = local_data.getSliceOn(corner, {0, 0});
	View<double, 0>       local_ghosts = local_data.getGhostSliceOn(corner, {0, 0});
	local_ghosts[{}] += -local_slice[{}] / 3.0;
...
void BiLinearGhostFiller::fillGhostCellsForLocalPatch(const PatchInfo<2> &pinfo, const std::vector<ComponentView<const double, 2>> &local_datas) const
{
