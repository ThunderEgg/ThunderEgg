...
public:
   ...
   */
  template<int M>
  inline View<double, M + 1> getSliceOn(Face<D, M> f, const std::array<int, D - M>& offset)
  {
  ...
  inline const double& operator[](const std::array<int, D + 1>& coord) const { return view[coord]; }
  ...
  inline void set(const std::array<int, D + 1>& coord, double value) { view.set(coord, value); }

  /**
   * @brief Get the strides of the patch in each direction
   */
  inline const std::array<int, D + 1>& getStrides() const { return view.getStrides(); }
  /**
   * @brief Get the coordinate of the first element
   */
  inline const std::array<int, D + 1>& getStart() const { return view.getStart(); }
  /**
   * @brief Get the coordinate of the last element
   */
  inline const std::array<int, D + 1>& getEnd() const { return view.getEnd(); }
  /**
   * @brief Get the coordinate of the first ghost cell element
   */
  inline const std::array<int, D + 1>& getGhostStart() const { return view.getGhostStart(); }
  /**
   * @brief Get the coordinate of the last ghost cell element
   */
  inline const std::array<int, D + 1>& getGhostEnd() const { return view.getGhostEnd(); }
  /**
   * @brief Get the View for the array
   *
   * @return const PatchView<double, D>& the View
   */
  const PatchView<double, D>& getView() { return view; }
};
