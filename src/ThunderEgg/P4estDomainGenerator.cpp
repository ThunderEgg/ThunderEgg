...
void
IterVolumeWrap(p4est_iter_volume_info_t* info, void* user_data)
{
...
void
IterFaceWrap(p4est_iter_face_info_t* info, void* user_data)
{
  const IterateFunctions* functions = (IterateFunctions*)user_data;
...
void
IterCornerWrap(p4est_iter_corner_info_t* info, void* user_data)
{
  const IterateFunctions* functions = (IterateFunctions*)user_data;
...
void
p4est_iterate_ext(p4est_t* p4est,
...
int
CoarsenWrap(p4est_t* p4est, p4est_topidx_t which_tree, p4est_quadrant_t* quadrants[])
{
...
p4est_coarsen_ext(
  ...
  const std::function<void(p4est_topidx_t, int, p4est_quadrant_t*[], int, p4est_quadrant_t*[])>&
    replace)
{
...
void
InitData(p4est_t*, p4est_topidx_t, p4est_quadrant_t* quadrant)
{
 ...
 * constructor
 */
P4estDomainGenerator::P4estDomainGenerator(p4est_t* p4est,
...
P4estDomainGenerator::operator=(const P4estDomainGenerator& other)
{
  domain_patches = other.domain_patches;
  ns = other.ns;
  num_ghost_cells = other.num_ghost_cells;
...
void
P4estDomainGenerator::extractLevel()
{
...
void
P4estDomainGenerator::coarsenTree()
{
  p4est_coarsen_ext(
    my_p4est,
    false,
...
void
P4estDomainGenerator::createPatchInfos()
{
  ...
  p4est_iterate_ext(
    ...
    [&](const p4est_iter_volume_info_t* info) {
      ...
      double upper_unit_x =
        (double)(info->quad->x + P4EST_QUADRANT_LEN(info->quad->level)) / P4EST_ROOT_LEN;
      double upper_unit_y =
        (double)(info->quad->y + P4EST_QUADRANT_LEN(info->quad->level)) / P4EST_ROOT_LEN;
 ...
 */
void
addCoarseNbrInfo(const p4est_iter_face_side_t& side1,
                 const p4est_iter_face_side_t& side2,
 ...
 */
void
addFineNbrInfo(const p4est_iter_face_side_t& side1,
               const p4est_iter_face_side_t& side2,
               const Data* ghost_data)
{
 ...
 */
void
addNormalNbrInfo(const p4est_iter_face_side_t& side1,
                 const p4est_iter_face_side_t& side2,
                 const Data* ghost_data)
{
  if (!side1.is.full.is_ghost) {
 ...
 */
void
addNbrInfo(const p4est_iter_face_side_t& side1,
           const p4est_iter_face_side_t& side2,
           const Data* ghost_data)
{
  if (side1.is_hanging) {
 ...
 */
void
addCornerFineNbrInfo(const p4est_iter_corner_side_t& side1,
                     const p4est_iter_corner_side_t& side2,
                     const Data* ghost_data)
{
  if (!side1.is_ghost) {
 ...
 */
void
addCornerCoarseNbrInfo(const p4est_iter_corner_side_t& side1,
                       const p4est_iter_corner_side_t& side2,
                       const Data* ghost_data)
{
  if (!side1.is_ghost) {
 ...
 */
void
addCornerNormalNbrInfo(const p4est_iter_corner_side_t& side1,
                       const p4est_iter_corner_side_t& side2,
                       const Data* ghost_data)
{
  if (!side1.is_ghost) {
 ...
 */
void
addCornerNbrInfo(const p4est_iter_corner_side_t& side1,
                 const p4est_iter_corner_side_t& side2,
                 const Data* ghost_data)
{
...
void
P4estDomainGenerator::linkNeighbors()
{
      ...
      if (info->sides.elem_count == 4) {
        for (Corner<2> c : Corner<2>::getValues()) {
          p4est_iter_corner_side_t side1 =
            ((p4est_iter_corner_side_t*)info->sides.array)[c.getIndex()];
          p4est_iter_corner_side_t side2 =
            ((p4est_iter_corner_side_t*)info->sides.array)[c.opposite().getIndex()];
...
void
P4estDomainGenerator::updateParentRanksOfPreviousDomain()
{
...
Domain<2>
P4estDomainGenerator::getCoarserDomain()
{
  if (curr_level >= 0) {
    extractLevel();
  }
...
Domain<2>
P4estDomainGenerator::getFinestDomain()
{
  if (curr_level >= 0) {
    extractLevel();
  }
