				...
				for (int c = 0; c < local_datas.size(); c++) {
					...
					nested_loop<1>(nbr_ghosts.getStart(), nbr_ghosts.getEnd(),
					               [&](const std::array<int, 1> &coord) {
						               nbr_ghosts[coord] = local_slice[coord];
					               });
				}
			} break;
			case NbrType::Coarse: {
				...
				for (int c = 0; c < local_datas.size(); c++) {
					...
					nested_loop<1>(nbr_ghosts.getStart(), nbr_ghosts.getEnd(),
					               [&](const std::array<int, 1> &coord) {
						               nbr_ghosts[{(coord[0] + offset) / 2}]
						               += 2.0 / 3.0 * local_slice[coord];
					               });
				}
			} break;
			case NbrType::Fine: {
			...
			for (Side<2> side : Side<2>::getValues()) {
				if (pinfo->hasNbr(side)) {
					switch (pinfo->getNbrType(side)) {
						...
						case NbrType::Coarse: {
							auto local_slice  = local_datas[c].getSliceOnSide(side);
							auto local_ghosts = local_datas[c].getGhostSliceOnSide(side, 1);
							int  offset       = 0;
							if (pinfo->getCoarseNbrInfo(side).orth_on_coarse
							    == Orthant<1>::upper()) {
								offset = pinfo->ns[!side.getAxisIndex()];
							}
							nested_loop<1>(
