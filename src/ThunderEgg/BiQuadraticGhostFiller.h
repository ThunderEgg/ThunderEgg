...
class BiQuadraticGhostFiller : public MPIGhostFiller<2>
{
	 ...
	 */
	BiQuadraticGhostFiller(std::shared_ptr<const Domain<2>> domain_in) : MPIGhostFiller<2>(domain_in, 1) {}
	...
	void fillGhostCellsForLocalPatch(const PatchInfo<2> &pinfo, const std::vector<LocalData<2>> &local_datas) const override;
};
} // namespace ThunderEgg
#endif
