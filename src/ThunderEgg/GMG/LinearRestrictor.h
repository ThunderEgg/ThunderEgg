...
template <int D> class LinearRestrictor : public MPIRestrictor<D>
{
	...
	void extrapolateBoundaries(const PatchInfo<D> &pinfo, ComponentView<const double, D> &fine_data, ComponentView<double, D> &coarse_data) const
	{
		...
		for (size_t i = 0; i < D; i++) {
			starts[i] = orth.isLowerOnAxis(i) ? 0 : coarse_data.getEnd()[i] + 1;
		}
	...
	{
		auto coarse_local_datas = coarser_vector->getComponentViews(parent_index);
		auto fine_datas         = finer_vector->getComponentViews(pinfo.local_index);
		// get starting index in coarser patch
		Orthant<D>         orth = pinfo.orth_on_parent;
		std::array<int, D> starts;
		for (size_t i = 0; i < D; i++) {
			starts[i] = orth.isLowerOnAxis(i) ? 0 : coarse_local_datas[0].getEnd()[i] + 1;
		}
