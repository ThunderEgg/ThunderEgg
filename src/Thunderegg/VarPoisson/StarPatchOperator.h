	                        ...
	                        std::shared_ptr<const Vector<D - 1>> gamma, LocalData<D> f) override
	{
		...
		loop<0, D - 1>([&](int axis) {
			...
			if (sinfo.pinfo->hasNbr(lower_side)) {
				...
				nested_loop<D - 1>(mid.getStart(), mid.getEnd(), [&](std::array<int, D - 1> coord) {
					f_slice[coord] += (upper[coord] - 3 * mid[coord]) / h2[axis];
				});
			}
			// middle
			{
			...
			if (sinfo.pinfo->hasNbr(upper_side)) {
				...
				/*
				                nested_loop<D - 1>(mid.getStart(), mid.getEnd(), [&](std::array<int,
				   D - 1> coord) { f_slice[coord] += (c_mid[coord] + c_lower[coord]) * (lower[coord]
				   - mid[coord]) / (2 * h2[axis]);
				                });
				                */
				nested_loop<D - 1>(mid.getStart(), mid.getEnd(), [&](std::array<int, D - 1> coord) {
